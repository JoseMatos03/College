...
class Balde():
    ...
    def cria_grafo(self):
        ...
        while estados != []:
            ...
            for e in expansao:
                self.g.add_edge(estado, e, 1) # custo é 1 para todas as transições
    ...
    def enche1(self, nodo):
        res = f"({str(self.balde1)},{str(nodo[3])})"
        return res


    # Devolve o estado resultante de encher totalmente o segundo balde da torneira
    def enche2(self, nodo):
        res = f"({str(nodo[1])},{str(self.balde2)})"
        return res


    # Devolve o estado resultante de despejar o balde 1 no balde 2
    def despeja12(self, nodo):
        cap_atual1 = int(nodo[1])
        cap_atual2 = int(nodo[3])

        while (cap_atual1 > 0 and cap_atual2 < self.balde2):
            cap_atual1 -= 1
            cap_atual2 += 1

        res = f"({str(cap_atual1)},{str(cap_atual2)})"
        return res
    ...
    def despeja21(self, nodo):
        ...
        while (cap_atual1 < self.balde1 and cap_atual2 > 0):
            cap_atual1 += 1
            cap_atual2 -= 1
